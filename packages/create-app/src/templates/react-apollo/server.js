...
export async function createServer(
  ...
  app.use('*', async (req, res) => {
    try {
      ...
      const { html, styles, apolloState } = await render(req, url, routerContext, helmetContext);
      ...
      const headOutlet = `
        ...
        <script>
          window.__IS_READY__ = true;
          window.__APOLLO_STATE__ = ${JSON.stringify(apolloState).replace(/</g, '\\u003c')};
        </script>
      `;
      const rootOutlet = html;

      res
        .status(200)
        .set({ 'Content-Type': 'text/html' })
        .end(template
          .replace('<!--head-outlet-->', headOutlet)
          .replace(`<!--root-outlet-->`, rootOutlet));
    } catch (err) {
