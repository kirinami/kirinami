...
export async function getUsersWithPagination(search?: string, page = 1, size = 10) {
...
export async function getUserByIdForUser(user: User, id?: number) {
  id = id || user.id;
...
export async function createUser(input: CreateUserArgs['input']) {
  try {
...
export async function updateUserByUser(user: User, id: number, input: UpdateUserArgs['input']) {
  try {
    id = id || user.id;
