...
import { createTodoSchema, deleteTodoSchema, getTodosSchema, TodoSchema, updateTodoSchema } from './schemas';
...
export async function todosApi(fastify: FastifyInstance) {
  const app = fastify.withTypeProvider<ZodTypeProvider>();

  app.get('/', { schema: getTodosSchema }, async ({ headers, query }) => {
  ...
  app.post('/', { schema: createTodoSchema }, async ({ body }) => {
    await delay(100 + Math.random() * 1000);
  ...
  app.patch('/:id', { schema: updateTodoSchema }, async ({ params, body }) => {
    await delay(100 + Math.random() * 1000);
  ...
  app.delete('/:id', { schema: deleteTodoSchema }, async ({ params }) => {
    await delay(100 + Math.random() * 1000);
